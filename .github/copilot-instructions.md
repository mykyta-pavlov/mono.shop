...
- Architecture: Clean Architecture with separation into 3 projects: Core, Infrastructure, API projects
...
6. For aggregate/complex endpoints (summaries, heavy calculations, transactional flows): implement the logic in a dedicated service and expose a simple controller action that calls the service. Specifically:
    - Service interfaces belong in Core/Interfaces (e.g., Core.Interfaces.IBasketService).
    - Service implementations belong in Infrastructure/Services (e.g., Infrastructure.Services.BasketService).
    - DTOs for the HTTP surface belong in API/Dtos — do not create DTOs in Core or Infrastructure. Services should return domain models (Core.Models.*) or plain objects; Controllers map domain models to API/Dtos via AutoMapper or manual mapping.
    - Controllers should depend only on Core interfaces and remain thin — avoid grouping, heavy calculations, or multiple repository calls inline.
